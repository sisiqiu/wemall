...
public class CCPSDK {
	...
	private String accountSID = "8a216da85e5b3555015e5b8b90d50094";
	private String accountToken = "ea7c4a13390941c2b6cd77c128559223";
	public String appID = "8aaf07085e5b2f11015e5bb1a81900be";
	private BodyType BODY_TYPE = BodyType.Type_JSON;

	/** 唯一实例 */
	private static CCPSDK myInstance;
	 ...
	 * @return
	 */
    public static CCPSDK getInstance() {
    	if (myInstance == null){
    		myInstance = new CCPSDK();
    	}
    	// 此处每次发短信调用
    	props = myInstance.getSettings();
//    	myInstance.init("sandboxapp.cloopen.com", "8883");
//    	myInstance.setAccount("8a48b55149f50bf3014a090834710de1", "23f748f3e380445b805c433795e4ca44");
//    	myInstance.setAppId("8a48b55149f50bf3014a090b1f5f0df2");
    	return myInstance;
    }
    ...
    public Properties getSettings() {
    	if (props == null) {
    		// 装载参数
//    		props = SmsConfig.getInstance().getProperties();

    		// 初始化
    		if (props == null || props.size() == 0 || !props.containsKey("ccp.appID")) {
    			if (props == null)
    				props = new Properties();
    			props.setProperty("ccp.serverIP", Global.getConfig("ccp.serverIP"));
    			props.setProperty("ccp.serverPort", Global.getConfig("ccp.serverPort"));
    			props.setProperty("ccp.accountSID", Global.getConfig("ccp.accountSID"));
    			props.setProperty("ccp.accountToken", Global.getConfig("ccp.accountToken"));
    			props.setProperty("ccp.appID",  Global.getConfig("ccp.appID"));
    		} else {
	    		try {
	    			serverIP = props.getProperty("ccp.serverIP", serverIP);
	    			serverPort = props.getProperty("ccp.serverPort", serverPort);
	    			accountSID = props.getProperty("ccp.accountSID", accountSID); 			
	    			accountToken = props.getProperty("ccp.accountToken", accountToken);
	    			appID = props.getProperty("ccp.appID", appID);
	    		} catch (Exception ex) {
	    			ex.printStackTrace();
	    		}
    		}
...
*/
	...
	private HashMap<String, Object> json2Map(String result){ 
