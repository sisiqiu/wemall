 ...
 */
package com.fulltl.wemall.modules.cms.utils;
...
public class CmsUtils {
	 ...
	 * @return
	 */
	public static Article getArticleData(String articleId){
	 ...
	 * ${fnc:getArticleList(category.site.id, category.id, not empty pageSize?pageSize:8, 'posid:2, orderBy: \"hits desc\"')}"
	 */
	public static List<Article> getArticleList(String siteId, String categoryId){
    ...
    public static void addViewConfigAttribute(Model model, Category category){
        ...
        boolean goon = true;
        do{
            if(c == null || c.getParent() == null || c.getParent().isRoot()){
                goon = false;
            }
            if (c != null) {
                categoryList.add(c);
