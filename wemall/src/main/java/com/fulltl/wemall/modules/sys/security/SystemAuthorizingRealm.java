...
import com.fulltl.wemall.modules.sys.web.LoginController;
...
public class SystemAuthorizingRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) {
        ...
        // 配置文件中不生效，先直接使用代码写死
		// 此处注意得设置加密算法如“SHA-1”和hash大小
        CustomCredentialsMatcher customMatcher = new CustomCredentialsMatcher();
        customMatcher.setHashAlgorithmName(SystemService.HASH_ALGORITHM);
        customMatcher.setHashIterations(SystemService.HASH_INTERATIONS);
		...
		// 密码验证
        String login_type = token.getLogin_type();
        // 如果是短信验证码登陆，则直接跳过密码验证环节
		// 校验登录验证码
		if (LoginController.isValidateCodeLogin(token.getUsername(), false, false) && !"SMSCode".equals(login_type)){
