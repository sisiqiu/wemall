...
<script type="text/javascript">
	...
	function ${input}SelectAction(fileUrl, data, allFiles){
		...
		$("#${input}").val(url);//</c:if>
		if(typeof(change${input}) == "function") {
			eval("change${input}(data, allFiles)");
		}
		${input}Preview();
		//top.$.jBox.close();
	}
	...
	function ${input}Preview(){
		...
		for (var i=0; i<urls.length; i++){
			if (urls[i]!=""){//<c:if test="${type eq 'thumb' || type eq 'images'}">
				...
				li = "<li><a href=\"${ctxFront}/download?filePath="+urls[i]+"\" url=\""+urls[i]+"\" target=\"_blank\">"+decodeURIComponent(urls[i].substring(urls[i].lastIndexOf("/")+1))+"</a>";//</c:if>
