...
<body>
	<ul class="nav nav-tabs">
		<li><a id="firstDis" href="${ctx}/wemall/wemallTimeDiscount/">限时打折活动列表</a></li>
		<li class="active"><a href="${ctx}/wemall/wemallTimeDiscount/form?id=${wemallTimeDiscount.id}">限时打折活动<shiro:hasPermission name="wemall:wemallTimeDiscount:edit">${not empty wemallTimeDiscount.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="wemall:wemallTimeDiscount:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="wemallTimeDiscount" action="${ctx}/wemall/wemallTimeDiscount/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="control-group">
			...
			<div class="controls">
				<input name="startDate" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate required"
					value="<fmt:formatDate value="${wemallTimeDiscount.startDate}" pattern="yyyy-MM-dd HH:mm:ss"/>"
		...
		<div class="control-group">
			...
			<div class="controls">
				<input name="endDate" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate required"
					value="<fmt:formatDate value="${wemallTimeDiscount.endDate}" pattern="yyyy-MM-dd HH:mm:ss"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/>
				<span class="help-inline"><font color="red">*</font> </span>
			</div>
		</div>
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:radiobuttons path="type" items="${fns:getDictList('percent_fixedValue')}" itemLabel="label" itemValue="value" htmlEscape="false" class="required"/>
				<span class="help-inline"><font color="red">*</font> </span>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">折扣量：</label>
			<div class="controls">
				<form:input path="discount" htmlEscape="false" maxlength="20" class="input-xlarge required"/>
				<span class="help-inline"><font color="red">*</font> </span>
			</div>
		</div>
		<div class="control-group">
		...
		<div class="form-actions">
			<shiro:hasPermission name="wemall:wemallTimeDiscount:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>
