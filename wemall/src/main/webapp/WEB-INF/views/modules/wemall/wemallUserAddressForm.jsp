...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/wemall/wemallUserAddress/">收货地址列表</a></li>
		<li class="active"><a href="${ctx}/wemall/wemallUserAddress/form?id=${wemallUserAddress.id}">收货地址<shiro:hasPermission name="wemall:wemallUserAddress:edit">${not empty wemallUserAddress.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="wemall:wemallUserAddress:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="wemallUserAddress" action="${ctx}/wemall/wemallUserAddress/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:radiobuttons path="isDefault" items="${fns:getDictList('yes_no')}" itemLabel="label" itemValue="value" htmlEscape="false" class="required"/>
				<span class="help-inline"><font color="red">*</font> </span>
			</div>
		</div>
		<div class="form-actions">
			<shiro:hasPermission name="wemall:wemallUserAddress:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>
